...
$(document).ready(function () {
	...
	$('a.deleteSession').click(function (event) {
		event.preventDefault();
		
		const url = $(this).attr('href');
		const movieElement = $(this).closest('.conf-step__seances-movie');
		
        $.ajax({
            url: url,
			success: function (data) {
				alert('Фильм успешно удалён из сетки сеансов');
				movieElement.remove();
				//window.location.href = 'admin';
			},
			error: function () {
	...
	$('form[name="startSale"]').submit(function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				alert('Продажа билетов успешно открыта!');
				// Дополнительная обработка, если нужно
			},
			error: function (xhr, status, error) {
				console.error('Ошибка при открытии продажи билетов:', error);
				alert('Не удалось открыть продажу билетов. Попробуйте ещё раз.');
			}
		});
	});
})
